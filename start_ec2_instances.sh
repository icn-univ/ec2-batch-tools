...
echo "총 ${#INSTANCE_IDS[@]}대의 서버를 시작합니다:"
for ((i=0; i<${#INSTANCE_IDS[@]}; i++)); do
  echo "  - ${SERVER_NAMES[i]} (${INSTANCE_IDS[i]})"
done

# 확인 요청
read -p "위 서버들을 시작하시겠습니까? (y/n): " confirm
if [[ $confirm != [Yy]* ]]; then
  echo "작업이 취소되었습니다."
  exit 0
fi
...
# 서버 시작
echo "서버 시작 중..."
aws ec2 start-instances --region $AWS_REGION --instance-ids ${INSTANCE_IDS[@]} --output text > /dev/null

if [ $? -eq 0 ]; then
  echo "✓ 모든 서버 시작 명령이 완료되었습니다."
  for ((i=0; i<${#INSTANCE_IDS[@]}; i++)); do
    echo "  ✓ ${SERVER_NAMES[i]} (${INSTANCE_IDS[i]})"
  done
else
